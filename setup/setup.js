...
(function () {
  ...
  const I18N = {
    en: {
      title: "SeekClaw Setup",
      "welcome.title": "Welcome to SeekClaw",
      "welcome.subtitle": "SeekClaw is a one-click installer for OpenClaw",
      ...
      "config.keyNotice": "SeekClaw does not provide API keys. Please click the link to purchase one from the provider's website",
      ...
      "done.title": "All Set!",
      "done.subtitle": "SeekClaw is ready — switch providers or models anytime in Settings",
      ...
      "done.startFailed": "Gateway failed to start — please click Start SeekClaw to retry",
      "conflict.title": "Existing OpenClaw Detected",
      "conflict.subtitle": "An existing OpenClaw installation was found on your system, which may cause port conflicts with SeekClaw",
    ...
    zh: {
      title: "SeekClaw 安装引导",
      "welcome.title": "欢迎使用 SeekClaw",
      "welcome.subtitle": "SeekClaw 是 OpenClaw 的一键安装包",
      ...
      "config.keyNotice": "SeekClaw 不提供 API 密钥 请点击链接前往服务商官网购买 API 密钥后使用",
      "config.platform": "平台",
      "config.baseUrl": "接口地址",
      ...
      "done.title": "配置完成！",
      "done.subtitle": "SeekClaw 已就绪 随时可在设置中切换服务商或模型",
  ...
  async function checkExistingInstallation() {
    if (!window.seekclaw?.detectInstallation) {
      goToStep(1);
      return;
    }
    try {
      const res = await window.seekclaw.detectInstallation();
  ...
  async function handleUninstall() {
    ...
    try {
      const res = await window.seekclaw.resolveConflict({
        action: "uninstall",
  ...
  async function handleOAuthLogin() {
    ...
    try {
      ...
      var verifyResult = await window.seekclaw.verifyKey({
        provider: "moonshot",
      ...
      await window.seekclaw.saveConfig({
        provider: "moonshot",
        apiKey: result.accessToken,
        modelID: modelID,
        baseURL: "",
  ...
  async function handleVerify() {
    ...
    try {
      const result = await window.seekclaw.verifyKey(params);
      ...
      await window.seekclaw.saveConfig(buildSavePayload(params));
      setVerifying(false);
      goToStep(3);
    } catch (err) {
  ...
  async function handleComplete() {
    ...
    try {
      ...
      const result = await window.seekclaw.completeSetup(payload);
  ...
  function showRetryPortButton() {
    if (window.seekclaw?.retryRandomPort) {
  ...
  async function handleRetryPort() {
    ...
    try {
      const portResult = await window.seekclaw.retryRandomPort();
  ...
  async function loadLaunchAtLoginState() {
    if (!window.seekclaw?.setupGetLaunchAtLogin) {
      return;
    }
    try {
      const result = await window.seekclaw.setupGetLaunchAtLogin();
  ...
  function bindEvents() {
    ...
    els.platformLink.addEventListener("click", (e) => {
      ...
      if (url && window.seekclaw?.openExternal) {
