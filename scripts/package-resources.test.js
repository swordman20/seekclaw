...
test("Windows 全局 windowsHide 补丁应覆盖所有 spawn 调用", () => {
  ...
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "seekclaw-package-resources-"));
...
test("pruneNodeModules 应按扩展白名单裁剪并清理保留插件内部垃圾", () => {
  const sandbox = loadPackageResourcesSandbox();
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "seekclaw-package-prune-"));
...
test("verifyOutput 应要求基础扩展插件存在", () => {
  ...
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "seekclaw-package-verify-"));
