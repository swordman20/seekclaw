...
test("Windows afterPack wrapper 应优先调用 Helper.exe 并回退主 exe", () => {
  ...
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "seekclaw-afterpack-"));
  ...
  sandbox.replaceNodeBinary("win32", targetBase, "SeekClaw");
  ...
  assert.match(npmCmd, /SeekClaw Helper\.exe/);
  assert.match(npmCmd, /if exist/i);
  assert.match(npmCmd, /SeekClaw\.exe/);

  fs.rmSync(tmpRoot, { recursive: true, force: true });
});

test("Windows afterPack 应保留展开的 gateway node_modules 而不是生成 tar", async () => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "seekclaw-afterpack-flow-"));
