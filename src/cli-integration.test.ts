...
test("POSIX wrapper 应使用真实 Node.js（无 ELECTRON_RUN_AS_NODE）并注入 OPENCLAW_NO_RESPAWN", () => {
  const script = buildPosixWrapperForPaths("/Applications/SeekClaw/node", "/Applications/SeekClaw/openclaw.mjs");
...
test("Windows wrapper 应使用真实 Node.js（无 ELECTRON_RUN_AS_NODE）并注入 OPENCLAW_NO_RESPAWN", () => {
  const script = buildWinWrapperForPaths("C:\\SeekClaw\\node.exe", "C:\\SeekClaw\\openclaw.mjs");
...
test("Windows PATH 脚本中的 try/catch 不能被分号打断", () => {
  const script = buildWinPathEnvScript("add", "C:\\Users\\admin\\AppData\\Local\\SeekClaw\\bin");
...
test("Windows CLI 目录解析应同时返回当前路径与旧版迁移路径", () => {
  ...
  assert.equal(dirs.currentBinDir, "C:\\Users\\admin\\AppData\\Local\\SeekClaw\\bin");
