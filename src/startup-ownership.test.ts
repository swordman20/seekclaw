...
test("正常启动：seekclaw.config.json 完整 → seekclaw", async () => {
...
test("老用户升级：有 .device-id 无 seekclaw.config.json → legacy-seekclaw", async () => {
  const { detectOwnership } = await import("./seekclaw-config");
...
test("markSetupComplete 创建完整的 seekclaw.config.json", async () => {
  const { markSetupComplete, detectOwnership } = await import("./seekclaw-config");
  markSetupComplete();
  expect(detectOwnership()).toBe("seekclaw");
});
...
test("ensureDeviceId 无配置时自动创建", async () => {
  const { ensureDeviceId, readOneclawConfig } = await import("./seekclaw-config");
...
test("ensureDeviceId 已有配置时返回现有 ID", async () => {
  const { writeOneclawConfig, ensureDeviceId } = await import("./seekclaw-config");
