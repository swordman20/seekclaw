...
// 读取 registry URL（优先 seekclaw.config.json，兼容 legacy skill-store.json）
export function readSkillStoreRegistry(): string {
  const seekclawConfig = readSeekclawConfig();
  if (seekclawConfig?.skillStore?.registryUrl) {
    return seekclawConfig.skillStore.registryUrl;
  }
  const legacy = readLegacySkillStoreConfig();
  return typeof legacy?.registryUrl === "string" ? legacy.registryUrl : "";
}

// 写入 registry URL（写到 seekclaw.config.json + legacy 文件双写）
